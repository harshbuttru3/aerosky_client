const PilotVerify = require("../models/pilotVerifyModel");
...
const sendMail = async (name,email,id,token) => {
	const activationLink = `http://192.168.1.6:3000/submitPilotRequest/${id}?token=${token}`
  ...
  let mailOptions = {
      ...
      to: email,
      subject: `Activate Your Pilot Account!`,
      html: `
       <!DOCTYPE html>
		<html>
		<head>
		...
		<body>
		    <div id="container">
		        ...
		        <div id="content">
		            <p>Hello ${name}! Welcome to aero2astro, where we're pioneering a new era in the world of aviation, space modeling, and unmanned aerial vehicles (UAVs). We're thrilled that you've chosen to join us on this exciting journey!</p>
...
module.exports.createPilotVerify = async(req,res,next) => {
	try{
		const {name,
			email,
			image,
			number,
			location,
			previousWorkExperience} = req.body;
		const randomToken = await generateRandomToken(10);
		const emailVerify = await PilotVerify.create({
		...
		if(emailVerify){
			let result = await sendMail(name,email,emailVerify._id,randomToken);
...
module.exports.getPilotVerify = async(req,res,next) => {
	try{
		const {token,id} = req.body;
		console.log(token);
		const pilotVerify = await PilotVerify.findOne({_id:id,token:token})
		if(pilotVerify){
			const { email,name,
			image,
			number,
			location,
			previousWorkExperience } = pilotVerify
			const request = await PilotAccessRequest.create({
				email,
				name,
				image,
				number,
			...
			if(request){
				const result = await PilotVerify.findByIdAndRemove(id);
				console.log(result);
			    if(result){
