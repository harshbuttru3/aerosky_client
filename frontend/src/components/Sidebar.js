...
export default function Sidebar({ dragRef }) {
  ...
  const [userRefetchTrigger, setUserRefetchTrigger] = useRecoilState(
    userRefetchTriggerState
  );
  const [messagesRefetchTrigger, setMessagesRefetchTrigger] = useRecoilState(
    messagesRefetchTriggerState
  );
  const [arrivalMessage, setArrivalMessage] =
    useRecoilState(arrivalMessageState);
  const [currentMainTab, setCurrentMainTab] =
    useRecoilState(currentMainTabState);
  ...
  return (
    <main
      className={`${
        currentMainTab === "map"
          ? "w-[0%] overflow-hidden"
          : sideBarExtend
          ? "sm:w-[19%] xs:w-[14%] w-[100%] xs:relative fixed"
          : "w-[7%] relative"
      } 
    ...
    >
      <div
        ...
        className={`hidden xs:block absolute cursor-pointer ${
          currentMainTab === "map"
            ? "w-[0%] overflow-hidden"
            : sideBarExtend
            ? "h-6 w-6 -right-3 top-14 p-1"
            : "-right-2 top-12 h-5 w-5 p-1"
      ...
      >
        {menuItems.map((item, j) => (
          <Link
            href={`/${
              item?.menu?.toLowerCase() === "Dashboard"
                ? ""
                : item?.menu?.replace(" ", "")
            }`}
            onClick={() => {
              setCurrentTab(item.menu);
            }}
            className={`rounded-xl ${
              currentTab === item?.menu
                ? "bg-gradient-to-l to-sky-500 from-sky-600 shadow-lg shadow-sky-80/10 "
                : "bg-transparent"
            } 
          ...
          >
            <div
              className={`p-2 bg-white rounded-xl ${
                currentTab === item.menu ? "text-sky-500" : "text-black"
              } 
