...
export default function MapComponent({
	allProjects,currentTab,setCurrentTab
}) {
		...
		transition-all duration-200 ease-in-out bg-gray-300/60 flex  gap-1 flex items-center`}>
			<div className="md:w-[28%] w-full bg-[#1f1f1f] h-full overflow-y-auto scrollbar-thin">
				...
				<div className="flex flex-col gap-3 px-3 py-3">
					{
						allProjects?.map((project,k)=>(
							<MapProjectCard project={project} key={k} k={k} showProject={showProject}
			    ...
			    >
				    ...
				    {
				    	loadedKml?.map((kml,k)=>{
				    		return kml?.kml?.map((url,j)=>(
				    			<KmlComponent url={url} key={j}/>
				    		))
				    	})
				    }
