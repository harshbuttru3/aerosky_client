...
import ProjectTabMapCard from "./ProjectTabMapCard";
...
export default function ProjectTab({ clientId, currentClient }) {
  // body...
  ...
  const fetchUserProjects = async (user) => {
    const { data } = await axios.post(fetchAllProjectType, {
      // id:[...user?.projectsId,...user?.solarProjectsId,...user?.towerProjectsId]
      id: ["667a7b34516b161193782e0f"],
    });
    if (data.status) {
      setRequests(data?.project);
  ...
  useEffect(() => {
    if (currentClient && !projectsFetched) {
      fetchUserProjects(currentClient);
    }
  }, [currentClient]);
  ...
  return (
    <>
      ...
      <div className="w-full mt-3 flex items-center flex-wrap md:px-7 px-4 gap-3 overflow-x-auto scrollbar-none">
        <div
          ...
          >
            ...
            <div className="h-full w-full overflow-y-auto ">
              ...
              <div className="px-4 flex flex-col">
                ...
                <div
                  className="border-2 border-sky-400 hover:border-sky-600 transition-all duration-200 
							ease-in-out border-dashed  aspect-video rounded-lg flex items-center 
							justify-center cursor-pointer overflow-hidden relative z-50"
                >
                  <div
                    className={`h-full w-full ${
                      openLargeMap
                        ? "fixed top-0 left-0 z-[60] bg-white"
                        : "relative "
                    }
                ...
                <div className="mt-3 grid grid-cols-5 gap-3 w-full">
                  {currentProject?.kmlkmzFiles?.map((kml, j) => (
                    <ProjectTabMapCard
                      kml={kml}
                      j={j}
        ...
        {requests.map((req, j) => (
          ...
          >
            ...
            <div className="h-[1px] bg-gray-300 w-[100%] my-1" />
