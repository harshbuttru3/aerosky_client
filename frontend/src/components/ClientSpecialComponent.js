...
import ImageTagTab from './ClientSpecialComponents/ImageTagTab';
import AnalyseTab from './ClientSpecialComponents/AnalyseTab';
import UploadTab from './ClientSpecialComponents/UploadTab';
import DetailsTab from './ClientSpecialComponents/DetailsTab';
import SiteTab from './ClientSpecialComponents/SiteTab';
import ProcessedDataViewer from './ClientSpecialComponents/ProcessedDataViewer';
import exifr from 'exifr';
import {updateAnnotations} from '../utils/ApiRoutes';
import axios from 'axios';
import GenerateReport from '../components/ClientSpecialComponents/GenerateReport'
...
export default function ClientSpecialComponent() {
	...
	<>
		...
		ease-in-out w-full px-5 flex items-center justify-between py-3 bg-[#292929] z-30`}>
			<div className="flex items-center gap-3" >
				...
				<div className="flex items-center gap-2">
					...
					{
						( currentProject?.dataCollection?.toLowerCase() === 'not required' || !currentUser?.clientIndustry?.includes("Drone Service Provider") ) &&
						<>
							<div 
							...
							className={`px-3 py-2 text-gray-100 hover:bg-gray-700  transition-all text-sm duration-200 
							ease-in-out ${currentTab === 'report' ? 'border-blue-500 border-b-[1.4px] pb-[6.6px]' : 'pb-2 border-transparent'} cursor-pointer rounded-lg`}>
								Generate Report
							</div>
						</>
					}
		...
		</div>
		{
			...
			currentTab === 'report'?
			<GenerateReport project={currentProject} setProject={setCurrentProject}
			setCurrentTab={setCurrentTab} setCloseHeader={setCloseHeader}
			/>
			:
			currentTab === 'details'?
